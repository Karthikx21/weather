...
router.get("/air-quality", async (req, res): Promise<void> => {
  ...
  const cacheKey = `aq:${lat.toFixed(4)}:${lon.toFixed(4)}`;

  try {
    const result = await cache.getOrFetch(cacheKey, TTL.AIR_QUALITY, async () => {
      const url = new URL(`${AIR_QUALITY_BASE}/air-quality`);
      url.searchParams.set("latitude", String(lat));
      url.searchParams.set("longitude", String(lon));
      url.searchParams.set("current", ["pm10","pm2_5","carbon_monoxide","nitrogen_dioxide","ozone","sulphur_dioxide","dust","uv_index"].join(","));
      url.searchParams.set("hourly", ["pm2_5","pm10","ozone","nitrogen_dioxide"].join(","));
      url.searchParams.set("forecast_days", "1");
      url.searchParams.set("timezone", "auto");

      const response = await fetch(url.toString());
      if (!response.ok) throw Object.assign(new Error(`Air quality API error: ${response.status}`), { status: response.status });
      ...
      return {
        ...
        hourly: (hourly.time as string[]).slice(0, 24).map((time, i) => ({
          time,
    ...
    res.status(status).json({ error: "Failed to fetch air quality data" });
  }
});
